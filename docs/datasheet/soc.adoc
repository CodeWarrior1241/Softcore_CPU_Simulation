...
the **default** address space layout with the according memory capabilites (_physical memory attributes_ "PMAs"):
...
for the NEORV32's internal memory-mapped peripherals and I/O modules. The internal processor-internal <<_instruction_memory_imem>>
and <<_data_memory_dmem>> (if enabled), on the other hand, can be mapped to any address via the configuration generics.
